...
void Pawn::generateMoves(std::vector<std::unique_ptr<basePiece>> &board) {
    ...
    if(m_isWhite) {
        if(isSquareEmpty(board,m_position-10)) {m_possibleMoves.push_back(m_position - 10);}
        if(isSquareEmpty(board,m_position-20) && moveTwo && isSquareEmpty(board,m_position-10)) {
            m_possibleMoves.push_back(m_position - 20);
        }
        if(isOccupiedByEnemy(board, m_position - 11)){m_possibleMoves.push_back(m_position - 11);}
        if(isOccupiedByEnemy(board, m_position - 9)){m_possibleMoves.push_back(m_position - 9);}
    } else {
        if(isSquareEmpty(board,m_position + 10)) {m_possibleMoves.push_back(m_position + 10);}
        if(isSquareEmpty(board,m_position + 20) && moveTwo && isSquareEmpty(board,m_position + 10)) {
            m_possibleMoves.push_back(m_position + 20);
